...
bool Window::toggle_minimize() {
  if (!m_minimized) {
    bool buttonPanelVisibility;
    if (m_button_panel) {
      buttonPanelVisibility = m_button_panel->visible();
    }
    for (auto c: m_children) {
      c->set_visible(false);
    }
    if (m_button_panel) {
      m_button_panel->set_visible(buttonPanelVisibility);
    }
    if (!m_title.empty() || (m_button_panel && m_button_panel->visible())) {
      m_height_not_minimized = m_size[1] - m_theme->m_window_header_height;
      m_size = Vector2i(m_size[0], m_theme->m_window_header_height);
    } else {
      m_height_not_minimized = m_size[1];
      m_size = Vector2i(m_size[0], 0);
    }
  } else {
    bool buttonPanelVisibility;
    if (m_button_panel) {
    ...
    m_size[1] += m_height_not_minimized;
  }
